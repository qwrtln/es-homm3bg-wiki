---
search:
  exclude: true
---
#  :unit_ranged: unidad a distancia


## Notas

- [^1] Ranged units may move up to 1 space in Combat on the small battlefield. On the [large battlefield](../content/battlefield_expansion.md), their movement range is equal to their :initiative: value instead.
- [^1] Ranged units may attack first, and then move. On the [large battlefield](../content/battlefield_expansion.md) they can either move or attack.
- Si una unidad enemiga ocupa cualquier espacio adyacente a la unidad a distancia, la unidad a distancia solo puede atacar a la unidad adyacente.
- Si no hubiera una unidad enemiga adyacente a la unidad a distancia, la unidad a distancia puede atacar a una unidad enemiga en cualquier lugar del campo de batalla.
- [^1] Should the ranged attack originate in the back row of the battlefield and the attacked unit is in the opposite back row of the battlefield, a ranged penalty applies. On the [large battlefield](../content/battlefield_expansion.md), the ranged penalty applied when attacking a unit that is 8 or more hexes away.
- No hay represalias contra un ataque a distancia.Sin embargo, hay represalias si la unidad a distancia ataca una unidad que está adyacente a ella.


# Lista de unidades a distancia

- :bronze: [Elfos](../units/elves.md)
- :bronze: [Ojos Maléficos](../units/evil_eyes.md)
- :bronze: [Gremlins](../units/gremlins.md)
- :bronze: [Medianos](../units/halflings.md)
- :bronze: [Elementales de Hielo](../units/ice_elementals.md)
- :bronze: [Hombres Lagarto](../units/lizardmen.md)
- :bronze: [Magogs](../units/magogs.md)
- :bronze: [Tiradores](../units/marksmen.md)
- :bronze: [Orcos](../units/orcs.md)
- :bronze: [Lobos de Mar](../units/sea_dogs.md)
- :bronze: [Elementales de Tormenta](../units/storm_elementals.md)
- :silver: [Liches](../units/liches.md)
- :silver: [Magos](../units/magi.md)
- :silver: [Medusas](../units/medusas.md)
- :silver: [Tiradores Certeros](../units/sharpshooters.md)
- :silver: [Hechiceras](../units/sorceresses.md)
- :silver: [Fanáticos](../units/zealots.md)
- :golden: [Cíclopes](../units/cyclopes.md)
- :golden: [Enchanters](../units/enchanters.md)
- :golden: [Titanes](../units/titans.md)
- :azure: [Titanes](../units/titans.md)
- [Arrow Tower](../units/arrow_tower.md)


[^1]: Excepciones para modos de juego específicos. Esta explicación no es válida para todos los modos de juego. La variante específica para el modo de juego se menciona en el texto.
