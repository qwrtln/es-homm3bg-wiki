# Resistencia

=== "Regular"

    <figure markdown="span">
        ![Resistance](../assets/abilities-resistance.webp){ width="340" align=right }
    </figure>

=== "Potenciado"

    <figure markdown="span">
        ![Empowered Resistance](../assets/abilities-resistance-empowered.webp){ width="340" align=right }
    </figure>


___
<p style="text-align: center;" markdown>[Habilidad](index.md)</p>
___

| Type | Effect | :expert: Effect |
| :--- | :---: | :---: |
| Regular | :instant: Play this card immediately after the enemy casts a [spell](../spells/index.md). If the spell was cast with 1 :power: or less, ignore the [Spell](../spells/index.md) card's effect. | :instant: Play this card immediately after the enemy casts a [spell](../spells/index.md). Ignore the [Spell](../spells/index.md) card's effect. |
| Empowered | :instant: Play this card immediately after the enemy casts a [spell](../spells/index.md). Ignore the [Spell](../spells/index.md) card's effect. | - |


## Héroes con Habilidad de Inicio

- [:might: Bron](../heroes/bron.md)


## Notas

- El héroe principal del jugador debe participar en el combate.No es posible jugar resistencia si el héroe enemigo está en combate con unidades neutrales u otro jugador.
- La resistencia puede anular cualquier hechizo lanzado por el enemigo, no solo aquellos que se dirigen a unidades amigables.


## Viene Con

- [Juego Principal](../content/core_game.md)
- [Metas Ampliadas Regulares 2024](../content/regular_stretch_goals.md)
- [Naval Battles Expansion](../content/naval_battles_expansion.md) (Empowered)


## Ver También

- [Lista de Habilidades](index.md)
- [Lista de Hechizos](../spells/index.md)
