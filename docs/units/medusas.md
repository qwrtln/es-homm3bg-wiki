# Medusas

=== "Pocos"

    <figure markdown="span">
        ![Pocos Medusas](../assets/units-dungeon-silver-medusas-few.webp){ width="340" align=right }
    </figure>

=== "Manada"

    <figure markdown="span">
        ![Manada de Medusas](../assets/units-dungeon-silver-medusas-pack.webp){ width="340" align=right }
    </figure>

=== "Neutral"

    <figure markdown="span">
        ![Neutral Medusas](../assets/units-neutral-silver-medusas.webp){ width="340" align=right }
    </figure>

=== "Medusa Stores"

    <figure markdown="span">
        ![Medusa Stores Medusas](../assets/units-blank-silver.webp){ width="340" align=right }
    </figure>


| Statistics | Few | Pack | Neutral | Medusa Stores |
| :--- | :---: | :---: | :---: | :---: |
| Town | [Dungeon](../towns/dungeon.md) | [Dungeon](../towns/dungeon.md) | [Neutral](../towns/neutral.md) | - |
| Tier | :silver: | :silver: | :silver: | - |
| Type | [:unit_ranged:](index.md#ranged-units) | [:unit_ranged:](index.md#ranged-units) | [:unit_ranged:](index.md#ranged-units) | [:unit_ranged:](index.md#ranged-units) |
| :attack: | 3 | **4** | 3 | 3 |
| :defense: | 1 | 1 | 1 | 0 |
| :health_points: | 4 | 4 | 4 | 3 |
| :initiative: | 5 | **6** | 6 | 6 |
| Cost | 6 :gold: | 12 :gold: | 11 :gold: | - |
| Abilities | :unit_passive: After the Retaliation Attack, roll an [Attack die](../keywords/dice.md#attack-die), on a "0" the target is :paralysis:. | :unit_passive: Ignore the combat penalty against adjacent units.<br>:unit_retaliation: The target gains :paralysis:. | :unit_passive: Ignore the combat penalty against adjacent units.<br>:unit_retaliation: The target is :paralysis:. | :unit_attack: Ignore the Retalation Attack. If this unit is Stacked, the target gains [:paralysis:](../keywords/paralysis.md). |


## Notas

- ** Pocos ** - Después del ataque de represalia, rode *otro* dado.El resultado del dado rodado como parte del ataque no se aplica a la capacidad.
- **Medusa Stores** - "Ratalation" is a typo, it should say "Retaliation".
- Ver [Parálisis](../keywords/paralysis.md)


## Viene Con

- [Juego Principal](../content/core_game.md)
- [Naval Battles Expansion](../content/naval_battles_expansion.md) (Medusa Stores)


## Ver También

- [Medusa Stores (Creature Bank)](../fields/medusa_stores_creature_bank.md)
- [Lista de Unidades](index.md)
- [Lista de Ciudades](../towns/index.md)
