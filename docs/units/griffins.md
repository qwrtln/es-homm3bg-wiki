# Grifos

=== "Pocos"

    <figure markdown="span">
        ![Pocos Grifos](../assets/units-castle-bronze-griffins-few.webp){ width="340" align=right }
    </figure>

=== "Manada"

    <figure markdown="span">
        ![Manada de Grifos](../assets/units-castle-bronze-griffins-pack.webp){ width="340" align=right }
    </figure>

=== "Neutral"

    <figure markdown="span">
        ![Grifos Neutrales](../assets/units-neutral-bronze-griffins.webp){ width="340" align=right }
    </figure>

=== "Griffin Conservatory"

    <figure markdown="span">
        ![Griffin Conservatory Dwarves](../assets/units-blank-bronze.webp){ width="340" align=right }
    </figure>


| Statistics | Few | Pack | Neutral | Griffin Conservatory |
| :--- | :---: | :---: | :---: | :---: |
| Town | [Castle](../towns/castle.md) | [Castle](../towns/castle.md) | [Neutral](../towns/neutral.md) | - |
| Tier | :bronze: | :bronze: | :bronze: | - |
| Type | [:unit_flying:](index.md#flying-units) | [:unit_flying:](index.md#flying-units) | [:unit_flying:](index.md#flying-units) | [:unit_flying:](index.md#flying-units) |
| :attack: | 2 | **3** | 3 | 3 |
| :defense: | 0 | 0 | 0 | 0 |
| :health_points: | 4 | 4 | 4 | 4 |
| :initiative: | 6 | **9** | 8 | 8 |
| Cost | 4 :gold: | 6 :gold: | 7 :gold: | - |
| Abilities | :unit_retaliation: This unit can perform an unlimited number of Retaliation Attacks. | :unit_retaliation: This unit can perform an unlimited number of Retaliation Attacks. | :unit_retaliation: This unit can perform and unlimited number of Retaliation Attacks. | :unit_passive: This unit can perform an unlimited number of Retaliation Attacks. |


## Viene Con

- [Juego Principal](../content/core_game.md)
- [Naval Battles Expansion](../content/naval_battles_expansion.md) (Griffin Conservatory)


## Ver También

- [Griffin Conservatory (Creature Bank)](../fields/griffin_conservatory_creature_bank.md)
- [Lista de Unidades](index.md)
- [Lista de Ciudades](../towns/index.md)
